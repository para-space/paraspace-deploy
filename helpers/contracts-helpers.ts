...
export const withSaveAndVerify = async <C extends ContractFactory>(
  ...
  // if (INCREMENTAL_DEPLOYMENT && await hasContractAddressInDb(id)) {
  //   return
  // }
  ...
  let instance;
  if (libraries) {
    instance = await new factory(libraries, await getFirstSigner()).deploy(
      ...deployArgs,
      GLOBAL_OVERRIDES
    );
  } else {
    instance = await new factory(await getFirstSigner()).deploy(
      ...deployArgs,
      GLOBAL_OVERRIDES
    );
  }
